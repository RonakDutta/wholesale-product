...
const CartDrawer = () => {
	...
	// safety net in case the component unmounts mid-open
	useEffect(() => {
		return () => {
			document.body.style.overflow = "";
		};
	...
	return (
		<>
			<div
				className={`fixed inset-0 bg-slate-900/50 backdrop-blur-xs z-[60] transition-opacity duration-300 ${
					isVisible ? "opacity-100" : "opacity-0"
				}`}
				onClick={() => setIsCartOpen(false)}
			/>
			<div
				className={`fixed top-0 right-0 h-full w-full sm:w-104 bg-white shadow-2xl z-[70] flex flex-col transition-transform duration-300 ease-out ${
