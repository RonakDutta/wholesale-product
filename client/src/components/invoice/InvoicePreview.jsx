...
export default function InvoicePreview({ invoice, onClose, onSendEmail }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-3xl w-full max-h-[90vh] flex flex-col shadow-2xl animate-in fade-in zoom-in-95 duration-150">
        {/* Modal Top Bar */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handlePrint}
              className="p-2 text-slate-600 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
              title="Print Invoice"
            >
        ...
        <div className="p-8 overflow-y-auto flex-1 relative bg-white dark:bg-slate-900 print:p-0 print:overflow-visible">
          {/* Watermark Overlay */}
          ...
          <div className="flex justify-between items-start border-b-2 border-slate-900 pb-6 mb-6">
            <div>
              ...
              <p className="text-xs font-bold text-slate-500 mt-1 uppercase tracking-wider">
                {invoice.supplier_company || invoice.supplier_name || "Wholesale Merchant"}
              </p>
              {invoice.supplier_gstin && (
                <p className="text-xs text-slate-500">GSTIN: {invoice.supplier_gstin}</p>
              )}
            </div>
            ...
            <div className="text-right">
              ...
              <div className="text-xs text-slate-500 mt-1">
                Date: {invoice.issue_date ? new Date(invoice.issue_date).toLocaleDateString("en-IN") : "N/A"}
              </div>
              {invoice.due_date && (
          ...
          <div className="grid grid-cols-2 gap-6 mb-6 text-xs">
            <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-800">
              ...
              <p className="text-slate-600 dark:text-slate-400">Email: {invoice.supplier_email || "N/A"}</p>
              {invoice.supplier_upi_id && (
                <p className="text-slate-600 dark:text-slate-400 font-semibold text-blue-600">
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-800">
              ...
              <p className="text-slate-600 dark:text-slate-400 mt-1">GSTIN: {invoice.buyer_gstin || "N/A"}</p>
              <p className="text-slate-600 dark:text-slate-400">Phone: {invoice.buyer_phone || "N/A"}</p>
              <p className="text-slate-600 dark:text-slate-400">Email: {invoice.buyer_email || "N/A"}</p>
            </div>
          </div>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800 text-slate-700 dark:text-slate-300">
              {items.map((item, idx) => (
                <tr key={idx} className={idx % 2 === 0 ? "bg-slate-50/50 dark:bg-slate-800/30" : ""}>
                  ...
                  <td className="py-2.5 px-3 text-center font-mono text-slate-500">{item.hsn_code || "8504"}</td>
                  <td className="py-2.5 px-3 text-center font-bold">{item.quantity}</td>
                  <td className="py-2.5 px-3 text-right">₹{Number(item.unit_price).toFixed(2)}</td>
                  <td className="py-2.5 px-3 text-center">{item.gst_percent}%</td>
                  <td className="py-2.5 px-3 text-right">₹{Number(item.tax_amount).toFixed(2)}</td>
                  <td className="py-2.5 px-3 text-right font-bold">₹{Number(item.total).toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          {/* Totals Breakdown */}
          <div className="flex justify-between items-start pt-4 border-t border-slate-200 dark:border-slate-800 text-xs">
            ...
            <div className="w-64 space-y-1.5 text-right font-medium">
              ...
              <div className="flex justify-between text-base font-black text-slate-900 dark:text-white bg-slate-900 text-white p-2.5 rounded-xl mt-2">
