...
const createOrder = async (req, res) => {
  ...
  if (!products || products.length === 0 || !deliveryAddress || !totalAmount) {
    return res
      ...
      .json({
        success: false,
        message: "Missing required order checkout fields.",
      });
  }
  ...
  try {
    const cleanProductId = parseInt(
      String(targetItem.productId).split("#")[0],
      10,
    );
    const cleanSupplierId = parseInt(
      String(targetItem.supplierId || targetItem.vendorId).split("#")[0],
      10,
    );
    ...
    if (inventoryQuery.rows.length === 0) {
      return res.status(404).json({
        success: false,
        message:
          "Specified supplier product not found in active inventory catalogs.",
      });
    }
    ...
    return res
      .status(201)
      .json({ success: true, orderId: orderResult.rows[0].id });
  } catch (error) {
  ...
  try {
    ...
    if (orderRecord.buyer_id !== userId) {
      return res
        ...
        .json({
          success: false,
          message: "Access Denied: Unauthorized access verification layer.",
        });
    }
    ...
    console.error(
      "Error processing dynamic payment extraction routing:",
      error,
    );
    res
      .status(500)
      .json({
        success: false,
        message: "Internal server query resolution failure.",
      });
  }
};
...
const updatePaymentStatus = async (req, res) => {
  ...
  if (!["paid", "failed", "pending"].includes(paymentStatus)) {
    return res
      ...
      .json({
        success: false,
        message: "Invalid payload argument tracking variables.",
      });
  }
  ...
  try {
    ...
    if (checkOrder.rows.length === 0) {
      return res
        ...
        .json({
          success: false,
          message: "Order metadata mapping context missing.",
        });
    }
    if (checkOrder.rows[0].buyer_id !== userId) {
      return res
        ...
        .json({
          success: false,
          message: "Unauthorized credentials check state.",
        });
    }
    ...
    res.json({
      success: true,
      message: `Payment state mapped successfully to active parameter: ${paymentStatus}.`,
    });
  } catch (error) {
    ...
    res
      ...
      .json({
        success: false,
        message: "Internal server state error processing updates.",
      });
  }
};
